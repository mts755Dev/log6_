...
serve(async (req) => {
  ...
  try {
    ...
    const limit = typeof payload?.limit === 'number' ? Math.min(Math.max(payload.limit, 1), 200) : 100;
    ...
    let query = supabase.from('payment_history').select('*').order('created_at', { ascending: false }).limit(limit);
